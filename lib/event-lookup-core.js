...
function locationMatches(event, location) {
  ...
  const wantedCountries = new Set([location.countryCode, location.country].map(normalize).filter(Boolean))
  const eventCountries = [event.countryCode, event.country].map(normalize).filter(Boolean)
