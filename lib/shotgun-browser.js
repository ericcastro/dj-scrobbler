...
async function withinDeadline(promise, deadline, message = 'Shotgun browser search timed out') {
...
async function runSearch(artist, options = {}) {
  ...
  try {
    await withinDeadline(
      window.loadURL(`https://shotgun.live/en/search?query=${encodeURIComponent(query)}`),
      deadline
    )
    await waitFor(window, 'Boolean(document.querySelector(\'input[type="text"], input[type="search"], input\'))', deadline)
    await delay(500)
    await withinDeadline(window.webContents.executeJavaScript(`(() => {
    ...
    return await withinDeadline(window.webContents.executeJavaScript(`(async () => {
...
module.exports = { searchShotgunInBrowser, withinDeadline }
