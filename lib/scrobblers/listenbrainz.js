...
function normaliseBaseUrl(raw) {
  ...
  // Forgive pastes of the full endpoint URL (.../1/submit-listens) or an
  // API-root URL (.../apis/listenbrainz/1, as Koito documents) — strip the
  // trailing API bits; we append '/1/submit-listens' ourselves.
  url.pathname = url.pathname.replace(/\/+$/, '').replace(/\/1(\/submit-listens)?$/i, '')
  url.search = ''
  url.hash = ''
...
function createListenBrainzScrobbler({ onStatus }) {
  ...
  return {
    ...
    async connect({ url, token }) {
      ...
      // Koito answers 400 "payload is nil" — but its auth middleware runs
      // BEFORE the payload check, so that specific 400 proves the token was
      // accepted (a bad token gets 401 and never reaches the handler).
      let res, errBody = ''
      try {
      ...
      const koitoAuthPassed = res.status === 400 && /payload/i.test(errBody)
      if (!res.ok && !koitoAuthPassed) {
        throw new Error(`Server returned ${res.status} — is this a ListenBrainz-compatible endpoint?`)
      }
