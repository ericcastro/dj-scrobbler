...
test('missing metadata offers local edits and exact title suggestions without blocking provider enrichment', () => {
  ...
  assert.match(appJs, /Not enough sources of metadata for this DJ set yet\./)
  assert.match(appJs, /Media seems too short to be a DJ set\. Refusing to look up any additional info\./)
  ...
  assert.match(appJs, /facts\.length === 0 && \(!noSoundCloudMatch \|\| suggestions\.length > 0\)/)
  assert.match(appJs, /class="[^"]*set-metadata-complete">add metadata<\/button>/)
  assert.match(appJs, /You can add metadata yourself if you like\./)
  ...
  assert.doesNotMatch(styleCss, /\.set-metadata-recovery\s*\{[^}]*justify-content:\s*space-between/s)
...
test('event lookup reports source-level progress and marquees long normalized summaries', () => {
  ...
  assert.match(appJs, /const EVENT_KAWAII_EMOTES = \[/)
  assert.match(appJs, /'〜\(￣▽￣〜\)'/)
  assert.match(appJs, /'♡_♡'/)
  assert.match(appJs, /'\(ﾉ\^ヮ\^\)ﾉ\*:\・ﾟ✧'/)
  assert.doesNotMatch(appJs, /┌\[ ◕ ◡ ◕\]┘/)
  assert.match(appJs, /function eventAnnouncementEmote\(result\)/)
  assert.match(appJs, /is playing in your city! <span class="set-event-emote">\$\{escHtml\(eventAnnouncementEmote\(result\)\)\}<\/span>/)
  assert.match(appJs, /class="set-event-emote"/)
  assert.match(appJs, /class="set-event-line set-event-detail-line"/)
  assert.match(appJs, /class="set-event-summary-text set-event-detail"/)
  assert.match(appJs, /class="set-event-separator" aria-hidden="true"/)
  assert.doesNotMatch(appJs, /<hr\s*\/?>/)
  ...
  assert.match(styleCss, /\.set-event-detail\s*\{[^}]*color:\s*var\(--text2\)[^}]*opacity:\s*\.48[^}]*font-size:\s*12px/s)
  assert.doesNotMatch(styleCss, /\.set-event-detail\s*\{[^}]*font-size:\s*90%/s)
  assert.match(styleCss, /\.set-event-emote\s*\{[^}]*color:\s*var\(--text2\)[^}]*opacity:\s*\.78/s)
  assert.match(styleCss, /\.set-event-detail-line\s*\{[^}]*margin-top:\s*-3px/s)
  assert.match(styleCss, /\.set-event-separator\s*\{[^}]*border-top:\s*1px solid/s)
  assert.match(styleCss, /\.set-event-change-location/)
  assert.match(styleCss, /\.set-event-location-control\s*\{[^}]*align-self:\s*stretch[^}]*justify-content:\s*space-between[^}]*opacity:\s*\.48/s)
...
test('local event suggestions can be dismissed from the header and re-enabled in Settings', () => {
  ...
  assert.match(styleCss, /\.set-event-dismiss\s*\{[^}]*opacity:\s*1/s)
...
test('mini-player keeps artist and track on one line with the set title beneath', () => {
  ...
  assert.match(appJs, /npTrackText\.textContent = data\.isId \? 'ID'/)
  assert.match(appJs, /npArtist\.textContent\s+= data\.isId \? 'ID'/)
