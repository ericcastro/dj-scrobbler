...
test('event lookup reports source-level progress and marquees long normalized summaries', () => {
  ...
  assert.match(appJs, /is playing in your city! \$\{escHtml\(eventAnnouncementEmote\(result\)\)\}/)
  assert.match(appJs, /class="set-event-detail"/)
  ...
  assert.match(styleCss, /\.set-event-detail\s*\{[^}]*color:\s*var\(--text2\)[^}]*opacity:\s*\.48/s)
